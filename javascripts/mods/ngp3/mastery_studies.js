var masteryStudies = {
	initCosts: {
		...
		ec: {13: 1.7777777777777776e55, 14: 1.7777777777777776e55},
		ec_legacy: {13: 1e55, 14: 1e55},
	...
	ecReqs: {
		13: function() {
			let start = player.aarexModifications.ngp3c?110e3:278e3
		...
		14: function() {
			let start = player.aarexModifications.ngp3c?225e3:180e3
	...
	timeStudyEffects: {
		...
		262: function(){
			let start = player.aarexModifications.ngp3c?5e4:5e4;
			let r = 1
			let exp = 1
			if (tmp.ngp3l) r = Math.max(player.resets / 15e3, 1)
			else {
				r = Math.max(player.resets / (start/1024), 1)
				exp = Math.sqrt(Math.max(player.resets / (start/12.1), 1))
			}
			if (r > 1e6) r = Math.pow(4 + Math.log10(r), 6)
			if (player.aarexModifications.newGameExpVersion) exp *= 2
			return Decimal.pow(r, exp)
		},
		263: function(){
			let x = player.meta.resets
			if (!tmp.ngp3l) x = x * (x + 10)
