...
function setupToDHTMLandData(){
	for (var c = 0; c < 3; c++) {
		...
		var branchUpgrades = ["Gain <span id='" + color + "UpgPow1'></span> " + color + " quark spins, but " + color + " quarks decay <span id='" + color + "UpgSpeed1'></span> faster.",
...
function doAfterResetCrunchStuff(g11MultShown){
	...
	if (!player.aarexModifications.ngp3c && speedrunMilestonesReached < 28 && (tmp.ngp3l || !player.achievements.includes("ng3p67"))) player.replicanti.galaxies = (player.timestudy.studies.includes(33)) ? Math.floor(player.replicanti.galaxies / 2) : 0
...
function replicantEggonUpdating(diff){
	var newBabies = tmp.twr.times(getEmperorDimensionMultiplier(1)).times(getReplicantSpeed()).times(tmp.ngp3c?5:1).times(diff/200)
